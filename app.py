...
st.write("PC에서는 원문 바로가기, 모바일에서는 네이버 검색으로 안전하게 연결됩니다.")
...
if st.button("🔄 최신 리포트 분석하기"):
    with st.spinner("🌐 데이터를 분석하는 중..."):
        try:
            ...
            st.success(f"총 {len(df)}개의 리포트를 분석했습니다! ({'모바일' if is_mobile else 'PC'} 환경으로 연결)")
                ...
                for stock in hot_stocks:
                    ...
                    with st.expander(f"📈 {stock} ({len(stock_data)}개 증권사 추천)"):
                        for _, row in stock_data.iterrows():
                            st.link_button(f"🏢 {row['증권사']} 리포트 확인", get_target_link(row), use_container_width=True)
            ...
            with col2:
                ...
                for _, row in up_reports.iterrows():
                    with st.container(border=True):
                        ...
                        st.link_button(f"⭐ {row['종목명']}: {row['리포트 제목']}", get_target_link(row), use_container_width=True)
            ...
            st.dataframe(
                df_display.drop(columns=["링크", "검색링크"]), 
